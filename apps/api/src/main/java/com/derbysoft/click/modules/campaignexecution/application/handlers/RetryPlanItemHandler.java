...
@Service
@Transactional
public class RetryPlanItemHandler {
    ...
    public RetryPlanItemHandler(PlanItemRepository planItemRepository,
    ...
    public PlanItem retry(UUID itemId, UUID tenantId, String reason, String triggeredBy) {
        PlanItem item = planItemRepository.findById(itemId)
            .orElseThrow(() -> new DomainError.NotFound("CE_404", "PlanItem not found: " + itemId));

        if (item.getStatus() != PlanItemStatus.FAILED) {
            throw new DomainError.Conflict("CE_409",
                "Can only retry FAILED items; current status: " + item.getStatus());
        }

        if (!item.canRetry()) {
            throw new DomainError.Conflict("CE_409",
                "Item cannot be retried (max attempts reached or permanent failure)");
        }
        ...
        WriteAction action = WriteAction.create(
            ...
            TriggerType.RETRY, triggeredBy, reason, now
        );
